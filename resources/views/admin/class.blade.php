                      ...
                      @foreach ($data as $d)
                        <tr>
                          ...
                          <td>
                            @if ($d->dokumen)
                              <p>{{ $d->dokumen }}</p>
                              <button class="btn btn-success mr-1"><a href="{{ asset('uploads/pdf/' . $d->dokumen) }}"
                                  target="_blank" rel="noopener noreferrer" class="text-reset ">Download
                                  File</a></button>
                            @endif
                          </td>
                          <td>
                            @if ($d->youtube)
                              <p>{{ OEmbed::get($d->youtube)->data()['title'] }}</p>
                              {!! OEmbed::get($d->youtube)->html(['width' => 400]) !!}
                            @endif
                          </td>
                          <td class="d-flex">
                            <button class="btn btn-warning mr-2 edit" data-id="{{ $d->id }}" data-toggle="modal"
  ...
  <div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <form class="" method="POST" action="" id="form" enctype="multipart/form-data">
          <div class="modal-body">
            <input type="hidden" name="" id="method">
            @csrf
            <div class="form-group">
              <label>Pertemuan</label>
              <input type="text" class="form-control @error('pertemuan') is-invalid @enderror" placeholder="Pertemuan"
                name="pertemuan" id="pertemuan">
              @error('pertemuan')
            ...
            <div class="form-group">
              <label>Materi</label>
              <input type="text" class="form-control @error('materi') is-invalid @enderror" placeholder="Materi"
            ...
            <div class="form-group">
              ...
              <input type="text" class="form-control @error('link') is-invalid @enderror" placeholder="Link Kelas"
                name="link" id="link">
              @error('link')
                <div class="invalid-feedback">
                  {{ $message }}
            ...
            <div class="form-group">
              ...
              <input type="text" class="form-control @error('youtube') is-invalid @enderror"
                placeholder="Link Video Youtube" name="youtube" id="youtube">
              @error('youtube')
                <div class="invalid-feedback">
                  {{ $message }}
                </div>
