                    ...
                    <tbody>
                      @foreach ($data as $d)
                        <tr>
                          ...
                          <td>{{ $d->created_at }}</td>
                          <td>
                            <a href="{{ asset('uploads/' . $d->payment->foto) }}" target="_blank"
                              rel="noopener noreferrer">
                              <img src="{{ asset('uploads/' . $d->payment->foto) }}" alt="" style="width: 100px">
                            </a>
                          </td>
                          <td class="d-flex">
                            <form action="/admin/verif/belum/{{ $d->id }}" method="POST">
                              @csrf
                              @method('PUT')
